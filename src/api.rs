...
fn handle_response(resp: ureq::http::Response<ureq::Body>) {
    ...
    write_record(&mut io::stdout().lock(), body.trim_end().as_bytes());
}

/// Writes one record and a newline to stdout, flushed so a consumer sees it as it lands.
/// A closed reader is a clean stop; any other failure is fatal, because output silently
/// lost under exit 0 is the worst thing this CLI can hand an agent.
        ...
        Ok(resp) => {
            ...
            loop {
                ...
                if let Some(data) = line.strip_prefix(b"data:") {
                    ...
                    // `data:` with no payload is a legal keep-alive, not a record; writing
                    // it put a blank line into a stream that promises one JSON per line.
                    if data.is_empty() {
                        continue;
                    }
                    // A closed reader (`bx answers … | head`) means nobody is left to
                    // stream to; draining the rest would burn minutes and quota.
            ...
            // Exit 0 with empty stdout is indistinguishable from a real answer that had
            // nothing to say — the worst signal this CLI can produce.
            if !emitted {
                eprintln!(
...
mod tests {
    ...
    fn streaming_agent_has_no_total_deadline() {
        ...
        // The wait for response headers inherits these two; unsetting both makes it
